 ...
 * @author joost
 */
public class WorkspaceFxRegistry implements WorkspaceRegistry {

    // commandfactory (global context) > to get current workspace
    private WorkspaceFxHandle activeWorkspace;
    private final List<Consumer<WorkspaceFxHandle>> activeWorkspaceListeners = new ArrayList<>();
    ...
    public WorkspaceFxRegistry(EditorView editorView) {
...
//        System.out.println("SCENE:" + mousePosition + "   EDITOR:" + editorView.sceneToLocal(mousePosition));
    ...
    public void add(WorkspaceHandle workspace) {
        if (workspace instanceof WorkspaceFxHandle fxWorkspace) {
    ...
    public void setOnActiveWorkspaceChanged(Consumer<WorkspaceFxHandle> listener) {
    ...
    public WorkspaceFxHandle active() {
